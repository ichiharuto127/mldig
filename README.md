...
[![CI](https://github.com/ichiharuto127/aidig/actions/workflows/ci.yml/badge.svg)](https://github.com/ichiharuto127/aidig/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/ichiharuto127/aidig.git
cd aidig
uv sync
...
```bash
uv run aidig            # 当日ダイジェストを digests/YYYY-MM-DD.md に生成
uv run aidig --dry-run  # LLMを呼ばず候補一覧のみ表示（無料）
uv run aidig --limit 5  # 要約本数を制限
```
...
| 2 | LangChain 導入：`aidig dig <arxiv-id>`（全文深掘り）→ 興味プロファイル推薦 | 設計済み |
